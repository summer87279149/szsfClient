...
@interface CompleteViewController ()<UITableViewDelegate,UITableViewDataSource,MallOrderTableViewCellDelegate>
{
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"已完成的商品订单";
...
-(void)GetRequestisComplete:(NSString *)isComplete isHeaderRefresh:(BOOL)isHeaderRefresh{
    ...
    WS(weakSelf)
    [SomeOtherRequest QueryOrderFormWithUserID:[YCUserModel userId] FormType:1 page:page isHeaderRefresh:isHeaderRefresh success:^(id response) {
        if (isHeaderRefresh) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cellUncompleteArr.count>0) {
        
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.delegate = self;
        cell.button.hidden = YES;
        cell.gotoCompleteButton.hidden = YES;
...
-(void)MallOrderCellBtnClickedComment:(UITableViewCell *)cell{
    NSIndexPath *index = [tableview indexPathForCell:cell];
    if (ArrayNonNull(cellUncompleteArr)){
        MallSectionModel *model = cellUncompleteArr[index.section];
        NSLog(@"当前点击的订单的模型数组内容是:%@",model.detailArr);
        MallOrderCellModel *model2 =model.detailArr[index.row];
        NSLog(@"评价这个商品的商品id是:%@",model2.productID);
    }
}
